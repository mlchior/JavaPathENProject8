...
public class TourGuideService {
	...
	public VisitedLocation trackUserLocation(User user) {
		VisitedLocation visitedLocation = createVisitedLocation(user);
		try {
			visitedLocation = gpsUtil.getUserLocation(user.getUserId());
		} catch (NumberFormatException nfe) {
			logger.error("Failed to get user location: invalid user ID format.", nfe);
		}
	...
	private VisitedLocation createVisitedLocation(User user) {
		Location location = new Location(generateRandomLatitude(), generateRandomLongitude());
		return new VisitedLocation(user.getUserId(), location, getRandomTime());
	}

	public void userLocation(User user) {
		gpsUtil.trackUserLocationAsync(user, this);
	}

	public List<Dto5NearAttractionByUser> getNearByAttractions(User user) {
		List<Attraction> attractions = gpsUtil.getAttractions();
		VisitedLocation lastLocation = user.getLastVisitedLocation();
		return attractions.parallelStream()
				.map(attraction -> new Dto5NearAttractionByUser(
						attraction.attractionName,
						attraction.latitude,
						attraction.longitude,
						lastLocation.location.latitude,
						lastLocation.location.longitude,
						rewardsService.getDistance(lastLocation.location, attraction),
						rewardsService.getRewardPoints(attraction, user)))
				.sorted(Comparator.comparing(Dto5NearAttractionByUser::getDistanceFromUser))
				.limit(5)
	...
	public List<UserLocationDto> getAllCurrentLocations() {
		return getAllUsers().parallelStream()
				.map(user -> {
					...
					return new UserLocationDto(user.getUserId().toString(), location);
				})
				.collect(Collectors.toList());
	}
